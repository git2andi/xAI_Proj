# -*- coding: utf-8 -*-
"""
Created on Sun Nov 26 18:14:12 2023

@author: Marius Ludwig Bachmeier

CNN for the PATHMNIST dataset
"""
...
parser.add_argument('--patience', type=int, default=8,
                    help='Number of patience epochs, default = 5')
parser.add_argument('--model_name', type=str, default="Alex_9_low_dropout",
                    help='Model name which will be part of the file names')
parser.add_argument('--dropout_value', type=float, default=0.2,
                    help='Dropout value that will be used')
...
def visclassdist(dif_cl, occs, desc_s, desc_s_2, colours="deepskyblue"):
    ...
    ax.set_ylabel('Occurences', fontsize = 30)
    ax.set_title('Class Distribution of the' + desc_s +
                 ' PathMNIST Dataset' + desc_s_2, fontsize = 40)
    for i, value in enumerate(occs):
        plt.text(i, value + 500, value, ha='center', va='top', fontsize = 24)
...
###################################
with np.load('./data/pathmnist_shuffled_kaggle.npz') as data:
...
print("Successful split into training and validation set\n")
...
#Loading Model########
######################
model = torch.hub.load('pytorch/vision:v0.10.0', 'alexnet', pretrained=True, dropout = dropout_value)
model.classifier[6] = nn.Linear(4096, num_classes)  # Adjust the final layer for 9 classes
model.to(device)
...
model = torch.hub.load('pytorch/vision:v0.10.0', 'alexnet', pretrained=False, dropout = dropout_value)
...
submission.to_csv(submission_path + model_name + '_submission.csv', index=False)
